...
export function AuthRedirect({ to = "/dashboard" }: { to?: string }) {
  ...
  return (
    <div className="linq-sheet-up fixed inset-x-0 bottom-6 z-50 flex justify-center px-4">
      <div className="flex w-full max-w-md flex-col gap-3 bg-surface px-5 py-4 shadow-lg ring-1 ring-line sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link href={to} className={buttonClasses({ size: "sm" })}>
            Go to dashboard
          </Link>
        </div>
      </div>
