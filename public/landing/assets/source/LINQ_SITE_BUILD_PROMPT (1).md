...
- **`US.svg`** and **`UK.svg`** (129×86, new) — added to cover the two most likely receive corridors, which were missing from the original World-Cup-squad flag set. **These are flat vector reconstructions, not extracted from the client's original art style** — the source flags are raster photo-style crops (see above) and these two couldn't be matched to that exact style without the client's source file. They're clean, correct, and sit fine in the stamp grid at the spec'd display sizes (78–104px wide), but a designer may want to redo them later to match the photographic treatment of the other 17 exactly. Swap freely — same 129×86 frame, same `.stamp` wrapper, no other change needed.
  - Total flag count is now **19**. Update Scene 02's headline number accordingly once the real corridor list is confirmed (see §10).
...
## 5. THE WAVE — AMBIENT BACKGROUND FOR THE INK SECTIONS

This is separate from the plane's trail (§4.1) and separate from the contrail described for scene 06 (§6, Scene 06). **The wave is a background texture that sits behind the near-black (`--ink`) sections of the whole page** — it's atmosphere, not a flight path. Think: the faint suggestion of moving air/current behind the content, always present, never competing with type.

**No wave SVG was supplied by the client** — build the placeholder below. If a real one arrives later, see the swap note at the end of this section.

### 5.1 Structure

- One `position: fixed`, full-viewport `<svg>` layer, `z-index: 2` (behind all content, in front of the flat `--ink` page background).
- **3 to 4 wave strokes**, each a single smooth `<path>` built from cubic bezier curves (a gentle, low-amplitude sine-like curve — not jagged, not decorative squiggle). Each layer spans the full viewport width and repeats/tiles vertically down the page so it's present behind every ink scene, not just one.
- Stagger the layers:
  - Layer 1 (furthest back): `stroke: var(--paper-dim)`, `opacity: 0.05`, `stroke-width: 1`, largest amplitude, slowest parallax.
  - Layer 2: `stroke: var(--signal)`, `opacity: 0.06`, `stroke-width: 1`, medium amplitude.
  - Layer 3 (closest): `stroke: var(--paper-dim)`, `opacity: 0.08`, `stroke-width: 1`, tightest amplitude, fastest parallax.
  - Optional layer 4: `stroke: var(--signal)`, `opacity: 0.04`, thinnest, barely-there.
- **This is texture, not a graphic.** If it's visible enough that someone would screenshot it on its own, it's too strong — turn the opacities down. It should read subliminally, the way a low hum of noise-grain would, not as "the site has a wave motif."

### 5.2 Motion

- Each layer parallax-scrolls **vertically** at a different rate (`0.1`–`0.3` of scroll delta, layer-dependent — furthest back moves least), giving a very slight sense of depth as the page scrolls past it. Driven by one lightweight `ScrollTrigger` with `scrub: 1`, not per-layer triggers.
- No independent looping animation on this layer — it moves only with scroll. (The plane's own idle drift, from §4.2, is the page's "something is alive" signal; the wave background should stay quiet so it doesn't compete.)
- The wave background is present behind **scenes 01, 02, 03, 06, 07, 08** (every `--ink` scene). It is **not** present behind scene 05 — when the page flips to full-bleed purple, the wave layer fades out over the same duration as the color wipe (§6, Scene 05) and fades back in as the page returns to ink. One less thing competing with the climax.
- `prefers-reduced-motion`: wave layers render static, no parallax, opacities unchanged.

### 5.3 If the client supplies a real wave SVG later

Extract its path `d` attribute(s), normalize to the viewBox above, and drop straight into the layer structure — same opacities, same parallax rates, same rule about staying behind scene 05. Don't redesign the system around his file; just re-skin the curve.

### 5.4 Do not confuse this with the plane's trail

The plane's motion trail (§4.1, `#plane-trail`) and the scene-06 contrail-on-a-line (§6, Scene 06) are part of the **plane system** — they exist because the plane is moving and are justified by §2's one rule ("nothing animates unless the plane's journey justifies it"). This wave layer is the one deliberate exception: it's ambient page texture, always present, not tied to the plane's position. Keep the two visually distinct — the trail is a hard-edged faceted ribbon in `--signal`; the wave background is a soft, near-invisible line at a fraction of the opacity.
...
- The 19 flags in an irregular grid across columns 5–12 — **not** a neat matrix. Passport-page logic: each flag sits at a slight random rotation (`-7°` to `+7°`, seeded/deterministic — same on every reload), with 2–3 deliberate empty cells so it reads as a used passport page, not a data table. **US and UK go in first** — first two stamps to land, top-left of the grid — since they're the corridors people will look for first.
...
2. On land, a **1px `--signal` border flashes** on that stamp for `120ms`, then fades to the `--paper-dim` border. This is the only purple in the scene — 19 tiny flashes.
3. Stagger `0.045`. All 19 land inside ~1.1s.
4. The whole grid gets a `2px` settle-shudder (`y: 2 → 0`, `0.1s`) when the last one lands.

`prefers-reduced-motion`: all 19 render immediately, no stamp animation, no flash. Grid still irregular.

⚠️ **FLAG SET DISCREPANCY — STILL PARTIALLY OPEN.** The 17 originally supplied flags are `ARG AUT BEL CIV COD COL CPV CRO ESP FRA GHA MAR NED NOR POR SEN SWE` — a **World Cup squad list, not a remittance-corridor list.** US and UK have now been added (see §10), covering the two corridors most likely to matter most. But the remaining 17 are still an unverified guess dressed up as a fact. **Do not ship a countries claim built from a football giveaway asset set.** Confirm the real supported-country list against what the bot actually accepts, drop any of the 17 that aren't real corridors, add any real ones that are missing, and only then lock the `19 COUNTRIES` headline to the true number.
...
- Target **60fps**. Profile scene 05 (the wipe) and scene 02 (19 simultaneous stamps) — those are the two that will drop frames.
...
| `US.svg`, `UK.svg` | 2 × 129×86 | Flags (added) | Flat vector, not photo-style — see note above |

### Still needed from the client ⚠️

1. **The real supported-country list.** The 17 original flags are a World Cup set (`ARG AUT BEL CIV COD COL CPV CRO ESP FRA GHA MAR NED NOR POR SEN SWE`) — US and UK have now been added, but the remaining 17 are still unverified as actual receive corridors. Scene 02's `19 COUNTRIES` claim depends on confirming this list against what the bot actually supports before launch.
...
5. **The wave SVG**, if he still wants his specific curve for the ink-section background — otherwise §5 builds a placeholder ambient wave.
...
- The ambient wave background layer (§5)
- `US.svg` / `UK.svg` flags (see §10)
